...
if ($abs_bdir =~ /^($abs_conf|$abs_model|$abs_cgi_bin)/)
{
...
#OMKLATAM
mkdir("$td/nmis8_menu",0755) or die "Cannot create $td/nmis8_menu: $!\n"; #/usr/local/nmis8/menu
system("cp -a /usr/local/nmis8/menu* nmis8_menu/ 2>/dev/null");
mkdir("$td/nmis8_lib",0755) or die "Cannot create $td/nmis8_lib: $!\n"; #/usr/local/nmis8/lib
system("cp -a /usr/local/nmis8/lib/* nmis8_lib/ 2>/dev/null");
mkdir("$td/nmis8_admin",0755) or die "Cannot create $td/nmis8_admin: $!\n"; #/usr/local/nmis8/admin
system("cp -a /usr/local/nmis8/admin/* nmis8_admin/ 2>/dev/null");
...
$status = system("tar","-rf",$backupfilename,"cron","nmis8_menu","nmis8_lib","nmis8_admin","omk_conf","omk_templates","omk_lib_json","omk_public_omk");
if ($status == -1)
{
	die "Failed to execute tar!\n";
}

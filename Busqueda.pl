#!/usr/bin/perl
#
## $Id: modelcheck.pl,v 2.1 2011/11/16 01:59:35 keiths Exp $
#
...
#########################################################################################################################
#Script Name    : Event search, using reguex
#Description    : Search for events in all logs in the directories /nmis8/logs/ and /omk/log/.
#########################################################################################################################

#  Add a new line in the file /usr/local/nmis8/conf/Config.nmis in the section of 'directories' => {
...
if ($arg{logs} eq "all"){
	...
	closedir DIR;
	for ( my $index = 0; $index <= $#dirlist; ++$index ) {
			if ( $dirlist[$index] =~ /^opCharts\.log/ ) {
			processLogFile(file => "$C->{omk_logs}/$dirlist[$index]");
			print "\n";
		}
	}
}
